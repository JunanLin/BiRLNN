...
class Trainer():
    ...
    def __init__(self, experiment_name, base_path=None):
        ...
        # Choose encoder automatically based on data filename prefix
        # If filename starts with 'SELFIES' use SELFIESEncoder, otherwise SMILESEncoder
        if self._config['DATA']['data'].upper().startswith('SELFIES'):
            self._encoder = SELFIESEncoder()
        else:
            self._encoder = SMILESEncoder()

        # Prepare starting token; for SELFIES we expect bracketed token like '[G]'
        start_tok = self._config['EVALUATION']['starting_token']
        if isinstance(self._encoder, SELFIESEncoder):
            if '[' not in start_tok:
                start_tok = f'[{start_tok}]'
        # Encoder.encode returns shape (1, L, T). Models expect a 1-D token
        # vector of length T. Extract a single token vector deterministically
        # (first token) and cast to float32. 
        enc_tok = self._encoder.encode([start_tok])
        if enc_tok.ndim == 3 and enc_tok.shape[1] > 0:
            self._starting_token = enc_tok[0, 0, :].astype('float32')
        elif enc_tok.ndim == 2 and enc_tok.shape[1] > 0:
            # defensive: some encoders may return (1, T)
            self._starting_token = enc_tok[0, :].astype('float32')
        else:
            # Raise an exception
            raise ValueError(f"Encoder produced no tokens for starting_token '{start_tok}'")
        ...
        if self._model_type == 'FBRNN':
            ...
            self._model = ForwardRNN(self._molecular_size, self._encoding_size,
                                     self._learning_rate, self._hidden_units, pad_index=None)
    ...
    def complete_run(self, stor_dir=None, restart=False):
        '''Training without validation on complete data'''

        # Create directories
        # resolve stor_dir
        if stor_dir is None:
            stor_dir = os.path.join(self._base_path, 'evaluation')
        elif not os.path.isabs(stor_dir):
            stor_dir = os.path.join(self._base_path, stor_dir)

        if not os.path.exists(os.path.join(stor_dir, self._experiment_name, 'models')):
            os.makedirs(os.path.join(stor_dir, self._experiment_name, 'models'))

        if not os.path.exists(os.path.join(stor_dir, self._experiment_name, 'molecules')):
            os.makedirs(os.path.join(stor_dir, self._experiment_name, 'molecules'))

        if not os.path.exists(os.path.join(stor_dir, self._experiment_name, 'statistic')):
            os.makedirs(os.path.join(stor_dir, self._experiment_name, 'statistic'))
        ...
        for i in range(self._epochs):
            ...
            if restart:
                tmp_stat_file = pd.read_csv(
                    os.path.join(stor_dir, self._experiment_name, 'statistic', 'stat_fold_' + str(fold) + '.csv'),
                ...
                if check_model(self._model_type, self._experiment_name, stor_dir, fold, i) and check_molecules(
                    ...
                    # Load model
                    self._model.build(
                        os.path.join(stor_dir, self._experiment_name, 'models', 'model_fold_' + str(fold) + '_epochs_' + str(i)))
            ...
            # Store model: only save the final epoch to reduce storage
            if i == (self._epochs - 1):
                self._model.save(os.path.join(stor_dir, self._experiment_name, 'models', 'model_fold_' + str(fold) + '_epochs_' + str(i)))

            # Sample new molecules
            new_molecules = []
            for s in range(self._samples):
                mol = self._encoder.decode(self._model.sample(self._starting_token, self._T))
                fmt = 'SELFIES' if isinstance(self._encoder, SELFIESEncoder) else 'SMILES'
                new_molecules.append(clean_molecule(mol[0], self._model_type, fmt=fmt))

            # Store new molecules
            new_molecules = np.array(new_molecules)
            pd.DataFrame(new_molecules).to_csv(
                os.path.join(stor_dir, self._experiment_name, 'molecules', 'molecule_fold_' + str(fold) + '_epochs_' + str(
                    i) + '.csv'), header=None)
    ...
    def single_run(self, stor_dir=None, restart=False):
        ...
        if not os.path.exists(os.path.join(stor_dir, self._experiment_name, 'validation')):
            os.makedirs(os.path.join(stor_dir, self._experiment_name, 'validation'))
        ...
        for i in range(self._epochs):
            ...
            if restart:
                ...
                tmp_val_file = pd.read_csv(
                    os.path.join(stor_dir, self._experiment_name, 'validation', 'val_fold_' + str(fold) + '.csv'),
                    header=None).to_numpy()
                tmp_stat_file = pd.read_csv(
                    os.path.join(stor_dir, self._experiment_name, 'statistic', 'stat_fold_' + str(fold) + '.csv'),
                    header=None).to_numpy()
            ...
            new_molecules = []
            for s in range(self._samples):
                sampled = self._model.sample(self._starting_token, self._T)
                mol = self._encoder.decode(sampled)
                fmt = 'SELFIES' if isinstance(self._encoder, SELFIESEncoder) else 'SMILES'
                new_molecules.append(clean_molecule(mol[0], self._model_type, fmt=fmt))
    ...
    def cross_validation(self, stor_dir=None, restart=False):
        ...
        for train, test in self._kf.split(self._data):
            ...
            for i in range(self._epochs):
                ...
                for s in range(self._samples):
                    mol = self._encoder.decode(self._model.sample(self._starting_token, self._T))
                    fmt = 'SELFIES' if isinstance(self._encoder, SELFIESEncoder) else 'SMILES'
                    cleaned = clean_molecule(mol[0], self._model_type, fmt=fmt)
                    new_molecules.append(cleaned)
                ...
                store_stat = np.array(tot_stat).reshape(i + 1, -1)
                pd.DataFrame(np.array(store_stat)).to_csv(
                    os.path.join(stor_dir, self._experiment_name, 'statistic', 'stat_fold_' + str(fold) + '.csv'), header=None)

                # Store validation data
                pd.DataFrame(np.array(tot_loss).reshape(-1, 1)).to_csv(
                    os.path.join(stor_dir, self._experiment_name, 'validation', 'val_fold_' + str(fold) + '.csv'), header=None)
